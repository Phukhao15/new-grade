...
// --- Type Definitions for Model Viewer (เพื่อให้ TS ไม่แจ้งเตือน) ---
declare global {
  namespace JSX {
...
const graduateInfo = {
  ...
  phone: "097-178-4484",
  lineId: "phu20453.",
  imageProfile: "/graduate-profile.jpg", // รูปโปรไฟล์ (ต้องมี / ข้างหน้าถ้าอยู่ใน public)
};
...
export default function Home() {
  const [comments, setComments] = useState<{ name: string; msg: string }[]>([]);
  const [inputName, setInputName] = useState("");
  const [inputMsg, setInputMsg] = useState("");
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  
  // State สำหรับเช็คว่าโมเดลโหลดเสร็จหรือยัง
  const [isModelLoaded, setIsModelLoaded] = useState(false);

  // --- Load Model Viewer Script (จำเป็นสำหรับแสดงโมเดล) ---
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // (ตรงนี้ถ้าจะต่อ Firebase ให้เอาโค้ด Firebase มาใส่นะครับ)
    setComments([{ name: inputName, msg: inputMsg }, ...comments]);
    fireFormalConfetti();
    setInputName("");
    setInputMsg("");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      {/* --- Header Section (เอารูปกลับมาแล้ว) --- */}
      <header className="bg-slate-900 text-white relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6 py-20 md:py-32 grid md:grid-cols-2 gap-12 items-center relative z-10">
          ...
          {/* Right: Profile Image (กลับมาแล้ว!) */}
          <motion.div 
          ...
          >
            ...
            <div className="aspect-[3/4] overflow-hidden rounded-lg shadow-2xl border-4 border-slate-800 relative group">
              <img 
                src={graduateInfo.imageProfile}
                alt="Graduate Profile" 
                className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
              />
              {/* Overlay เงาๆ เพิ่มมิติ */}
      ...
      {/* --- NEW SECTION: 3D Model Showcase (ย้ายมาตรงนี้) --- */}
          ...
          <div className="max-w-5xl mx-auto">
             <div className="text-center mb-10">
              ...
              <motion.div
                 ...
                 className="relative h-[500px] w-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex items-center justify-center"
              >
                  {/* Loading State */}
                  {!isModelLoaded && (
                    <div className="absolute z-0 flex flex-col items-center text-slate-500 animate-pulse">
                       <Loader2 className="w-10 h-10 animate-spin mb-2" />
                       <p className="text-sm font-medium">กำลังโหลดโมเดล 3D...</p>
                    </div>
                  )}
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12">
          <div>
            ...
            <div className="h-80 bg-slate-300 rounded-lg shadow-md overflow-hidden border border-slate-200 relative group">
              <iframe src="
https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3874.346280006492!2d100.5096033759347!3d13.81823348658059!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x30e29b9f54b53151%3A0x73d2b2a69752fd89!2z4Lio4Li54LiZ4Lii4LmM4Lie4Lij4Liw4LiZ4LiE4Lij4LmA4Lir4LiZ4Li34LitIOC4oeC4q-C4suC4p-C4tOC4l-C4ouC4suC4peC4seC4ouC5gOC4l-C4hOC5guC4meC5guC4peC4ouC4teC4o-C4suC4iuC4oeC4h-C4hOC4peC4nuC4o-C4sOC4meC4hOC4ow!5e0!3m2!1sth!2sth!4v1770817997114!5m2!1sth!2sth" width="100%" height="100%" style={{ border: 0 }} allowFullScreen={true} loading="lazy" className="grayscale group-hover:grayscale-0 transition-all duration-500"></iframe>
            </div>
      ...
      <section id="wishes" className="max-w-3xl mx-auto px-6 py-20">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="bg-white border border-slate-200 shadow-xl rounded-sm p-8 md:p-12">
          <form onSubmit={handleSubmit} className="mb-12">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="w-full md:w-auto px-8 py-3 bg-slate-900 text-white font-medium rounded-sm hover:bg-slate-800 transition-all flex items-center justify-center gap-2 mx-auto shadow-lg">
              <Send size={16} /> แสดงความยินดีกับบัณฑิต
            </motion.button>
          </form>
          <div className="border-t border-slate-200 pt-8">
            <h3 className="font-serif font-bold text-lg mb-6 text-slate-800">รายนามคนร่วมแสดงความยินดี ({comments.length})</h3>
            {comments.length === 0 ? (
              <p className="text-center text-slate-400 py-8 font-light italic">ยังไม่มีข้อความ</p>
            ) : (
              <ul className="space-y-0 divide-y divide-slate-100">
                {comments.map((c, i) => (
                  <motion.li key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="py-4 flex gap-4 items-start">
